// Wire contract for the extension<->server boundary. The durable kernel
// schema (strict objects, MAX_PARKING_LOT_ITEMS cap) lives in
// src/lib/cockpit/schema.ts; these DTOs stay deliberately permissive so a
// read never drops parked items in transit.
import { z } from "zod";
...
export function buildCockpitInputFromExtensionCapture(
  rawInput: z.input<typeof ExtensionCaptureInputSchema>,
): {
  message: string;
  mode: CockpitMode;
  sessionId?: string;
